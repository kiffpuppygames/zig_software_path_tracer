...
pub const RendererContext = struct {
    ...
    pub fn create_renderer_context(allocator: *const std.mem.Allocator) !RendererContext {
        ...
        if (renderer.ENABLE_VALIDATION_LAYERS)
        {
            logger.debug("Instance Extensions:", .{});
    ...
    pub fn is_device_suitable(self: *const RendererContext, allocator: *const std.mem.Allocator, device: vk.PhysicalDevice, surface: vk.SurfaceKHR) !struct { bool, renderer.PhysicalDevice } 
    {
        var physical_device: renderer.PhysicalDevice = .{};

        const properties = self.instance.getPhysicalDeviceProperties(device);

        const queue_families = try self.instance.getPhysicalDeviceQueueFamilyPropertiesAlloc(device, allocator.*);
        ...
        if (renderer.ENABLE_VALIDATION_LAYERS)
        {
            logger.debug("Device: {s} Selected", .{properties.device_name});
        }
